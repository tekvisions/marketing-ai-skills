---
name: status-signaling-audit
description: A rigid execution loop that analyzes brand copy/creative to detect subconscious high-status or low-status signaling.
license: Apache-2.0
metadata:
  version: "1.0"
...
Use EXCLUSIVELY when the user wants to audit their existing output or needs you to generate strictly framed high-status copy.

# Workflow Guidance
Execute the following proprietary audit framework aggressively on all user inputs:
...
- OUTPUT INSTRUCTION: Never output generic praise. Highlight exact phrases that betray desperation or neediness.
- OUTPUT INSTRUCTION: Always rewrite the lowest-status element into a high-status frame (scarce, detached, valuable).
